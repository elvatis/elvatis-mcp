...
async function haRequest(config: Config, method: 'GET' | 'POST', path: string, body?: unknown) {
...
export const homeTools = [
  {
    name: 'home_get_state',
    description: 'Get the current state of a Home Assistant entity (light, climate, sensor, switch, vacuum, media_player, etc.)',
    schema: z.object({
      entity_id: z.string().describe('Entity ID, e.g. light.wohnzimmer or climate.wohnzimmer'),
    }),
    handler: async (args: { entity_id: string }, config: Config) => {
  ...
  {
    name: 'home_light',
    description: 'Control a light: turn on/off/toggle, set brightness (0-100%), color temperature, or RGB color',
    schema: z.object({
  ...
  {
    name: 'home_climate',
    description: 'Control Tado thermostats: set target temperature or HVAC mode',
    schema: z.object({
...

  {
    ...
    handler: async (args: { room: string; scene: string }, config: Config) => {
  ...
  {
    name: 'home_vacuum',
    description: 'Control the Roborock vacuum: start full clean, stop, or return to dock',
    schema: z.object({
      action: z.enum(['start', 'stop', 'return_to_base', 'status']),
    }),
    handler: async (args: { action: string }, config: Config) => {
  ...
  {
    name: 'home_sensors',
    description: 'Read all environmental sensors: temperature, humidity, CO2 for all rooms',
    schema: z.object({}),
